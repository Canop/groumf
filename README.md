...
In this exemple, we want to replace words with links in html messages.
	...
	replacer.skipTags('a'); // we tell the replacer we don't want replacements inside a elements
	replacer.add("Some Name", 123456);
	...
	replacer.add("Pretty Name!", 654321);

	// replacements of an html message
	replacer.replace(domElement, function(name, id){ return '<a href="http://test.com/users/'+id+'">'+name+'</a>' });
...
-----------------------------------------------
	...
	(new Groumf).replace(document.body, /\w+/g, function(s){ return s.split('').reverse().join('') })
